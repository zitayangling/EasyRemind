    ...
    if (self) {
        ...
        switch (action) {
            case PasscodeActionSet:
                ...
                _enterPrompt = NSLocalizedString(@"请输入一个密码", nil);
                _confirmPrompt = NSLocalizedString(@"再次输入您的密码", nil);
                break;
...
- (void)loadView {
    ...
    contentView.backgroundColor = [UIColor colorWithRed:0.1 green:0.6 blue:0.5 alpha:1.0];
        ...
        case PasscodeActionSet:
            ...
            if (phase == 0) {
                ...
                if ([text isEqualToString:_passcode]) {
                    ...
                    messageLabel.text = NSLocalizedString(@"密码错误,请再试一次", nil);
                }
            }
            break;
...
- (void)showFailedAttempts {
    ...
    if (_failedAttempts == 1) {
        failedAttemptsLabel.text = NSLocalizedString(@"1次密码错误", nil);
    } else {
        failedAttemptsLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%d 次密码错误", nil), _failedAttempts];
    }
