...
@property (nonatomic, assign) CGFloat totalValue;///<总数
@property (nonatomic, assign) CGFloat radius;///<半径
@property (nonatomic, assign) CGFloat maxRadius;///<最大上限值
@property (nonatomic, assign) CGFloat startAngle;///<记录每个圆弧开始的角度
@property (nonatomic, assign) CFTimeInterval totalDuration;///<动画时长
@property (nonatomic, assign) CGFloat lineWidth;///<圆环线宽
@property (nonatomic, assign) NSInteger index;///<记录valueArray当前元素的下标
@property (nonatomic, assign) CGPoint centerPoint;///<记录当前path的中心点
@property (nonatomic, assign) CGFloat extendLength;///<半透明path延伸长度
@property (nonatomic, assign) CGPoint pieCenter;///<记录圆环中心
@property (nonatomic, assign) CGFloat originHeight;///<记录初始高度
@property (nonatomic, strong) NSMutableArray * startTimeArray;///<存储每个圆弧动画开始的时间
@property (nonatomic, strong) NSMutableArray * angelArray;///<记录每个path startAngle 和 endAngle, 数组里存的是NSDictionary
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        //类别名Label
        self.nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _radius, _radius * 0.3)];
        self.nameLabel.font = [UIFont boldSystemFontOfSize:20.f];
        self.nameLabel.textAlignment = NSTextAlignmentCenter;
        self.nameLabel.textColor = [UIColor orangeColor];
        self.nameLabel.center = self.pieCenter;
        //数值Label
...
- (void)removeAllSubLayers{
    ...
    for (CALayer * layer in subLayers) {
        if (layer != self.titleLabel.layer && layer != self.nameLabel.layer && layer != self.valueLabel.layer) {
    ...
    for (UIView * view in self.subviews) {
        if (view != self.titleLabel && view != self.nameLabel && view != self.valueLabel) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    for (NSInteger i = 0; i < self.angelArray.count; i++) {
        ...
        if (radian >= startAngle && radian < endAngle) {
            ...
            self.nameLabel.text = _nameArray[i];
            NSString *valueString = [NSString stringWithFormat:@"¥%@",_valueArray[i]];
            self.valueLabel.text = valueString;
//            _valueArray[i];
            NSLog(@"valueLabel====%@",self.nameLabel.text);
...
- (void)showTranslucencePathAction:(UITapGestureRecognizer *)sender{
    ...
    [self bringSubviewToFront:percentLabel];
    self.nameLabel.text = _nameArray[index];
...
- (void)creatPercentLabel{
    ...
    CGRect rect = [string stringWidthRectWithSize:CGSizeMake(0, 0) fontOfSize:16.f];
